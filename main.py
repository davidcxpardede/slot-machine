...
#Defining the Slot Machine Spin
...
#Displaying the Slot Machine Spin
...
#Requesting Number of Lines from User
...
#Requesting Bet Value from User 
def get_bet():
    while True:
...
def spin(balance):
    ...
    print(f"You won IDR {winnings}000.")
...
def main():
    balance = deposit()
    while True:
        print(f"Current balance is IDR {balance},000")
        answer = input("Press enter to play (q to quit) ")
        if answer == "q":
            break
        balance += spin(balance)
    
    print(f"You left with IDR {balance}000")
